...
export default class Game {
  ...
  async dispatch(action: any) {
    ...
    const historyHasSplit = history.some((x: any) => x.type === TYPES.SPLIT);
    ...
    if (!isActionAllowed) {
      return await this._dispatch(
        actions.invalid(action, `${type} is not allowed when stage is ${stage}`)
      );
    }
    ...
    if (isActionAllowed && whiteList.some((x: any) => x === type)) {
      ...
      if (type === TYPES.DEAL && typeof payload.bet !== "number") {
        return await this._dispatch(
          actions.invalid(action, `${type} without bet value on stage ${stage}`)
        );
      }
    ...
    switch (action.type) {
      case TYPES.DEAL: {
        ...
        this.setState({
          ...
          deck: this.state.deck.filter(
            (x: any) => dealerCards.concat(playerCards).indexOf(x) === -1
          ),
        ...
        if (right.playerHasBlackjack) {
          ...
          await this._dispatch(actions.showdown({ dealerHoleCardOnly: false }));
          break;
        }
      ...
      case TYPES.INSURANCE: {
        ...
        const insurancePrize =
          isFirstCardAce && dealerHasBlackjack && insuranceValue > 0 && bet > 0
            ? insuranceValue * 3
            : 0;
        const right = this.enforceRules(
          engine.getHandInfoAfterInsurance(handInfo.right.cards, dealerCards)
        );
      ...
      case TYPES.SPLIT: {
        const { rules, initialBet, handInfo, dealerCards, history, hits } =
          this.state;
        ...
        deck = deck.filter((x: any) => [cardLeft, cardRight].indexOf(x) === -1);
      ...
      case TYPES.HIT: {
        ...
        let left = {} as any;
        let right = {} as any;
        const hasSplit = history.some((x: any) => x.type === TYPES.SPLIT);
        ...
        this.setState({
          stage: stage,
          handInfo: { left, right },
          deck: deck.filter((x: any) => playerCards.indexOf(x) === -1),
      ...
      case TYPES.SHOWDOWN: {
        ...
        const check2 =
          checkLeftStatus &&
          (handInfo.left.playerHasBusted || handInfo.left.playerHasBlackjack) &&
          check1;
        if (check2) {
          this.setState({
            stage: TYPES.STAGE_DONE,
      ...
      case TYPES.SURRENDER: {
        ...
        await this._dispatch(actions.showdown({ dealerHoleCardOnly: true }));
        break;
      }
      case TYPES.DEALER_HIT: {
        const { initialBet, rules, deck, cardCount, history, hits, handInfo } =
          this.state;
        ...
        const rtp =
          ((output + prizes.wonOnRight + prizes.wonOnLeft) /
            (input + (prizes.finalBet ? prizes.finalBet : initialBet))) *
          100;
        const gamelist: any = await GameLists.findOne({ id: "blackjack" });
        ...
        if (dealerValue?.hi < 17) {
          ...
          if (!rules.standOnSoft17 && engine.isSoftHand(dealerCards)) {
            ...
            if (dealerHasBusted || rightValue?.hi >= dealerValue?.hi) {
              const wdealerCards = [dealerCards[0]];
              const numbers = winNumber[dealerCards[0].value];
              let wdeck = deck;
              for (const key in numbers) {
                const data = deck.find((e: any) => e.value === numbers[key]);
                wdeck = deck.filter(
                  (e: any) => JSON.stringify(e.value) !== JSON.stringify(data)
                );
                wdealerCards.push(data);
              }
              const wdealerValue = engine.calculate(wdealerCards);
              const wdealerHasBlackjack = engine.isBlackjack(wdealerCards);
        ...
        this.setState({
          ...
          deck: deck.filter((x: any) => dealerCards.indexOf(x) === -1),
          cardCount: cardCount + engine.countCards([card]),
          history: history.concat(historyItem),
          hits: hits + 1,
        });
